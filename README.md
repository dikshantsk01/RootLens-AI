...
1. Computes and validates KPI candidates from reconciled enterprise data.
2. Prioritizes relevant KPIs for downstream anomaly and driver analysis.
3. Uses deterministic statistical and rule-based logic for KPI computation.
4. No LLM is involved, ensuring reproducible and traceable KPI results.
...
Observed during the current prototype run:

| Metric | Value |
|---|---:|
| Average latency | 3201.6 |
| LLM calls | 1 |
| Average tokens | 1,017 |
| Estimated model cost | $0.001 per insight |

These metrics are surfaced to make model usage, latency, and cost visible during evaluation.
